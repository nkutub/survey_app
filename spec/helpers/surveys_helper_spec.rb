...
RSpec.describe SurveysHelper, type: :helper do
  describe 'survey_formatted_yes_percentage' do
    it 'should return the percentage of yes responses' do
      survey = create(:survey, :survey_with_50_percent_yes)
      expect(helper.survey_formatted_yes_percentage(survey)).to eq('50.0%')
    end
  end

  describe 'survey_formatted_no_percentage' do
    it 'should return the percentage of no responses' do
      survey = create(:survey, :survey_with_50_percent_no)
      expect(helper.survey_formatted_no_percentage(survey)).to eq('50.0%')
    end
  end

  describe 'survey_formatted_created_at' do
    it 'should return the formatted created at date' do
      survey = create(:survey, created_at: Time.zone.local(2021, 1, 1, 1, 0, 0))
      expect(helper.survey_formatted_created_at(survey)).to eq('January 01, 2021 at 01:00 AM')
    end
  end

  describe 'survey_formatted_response_created_at' do
    it 'should return the formatted created at date' do
      response = create(:response, created_at: Time.zone.local(2021, 1, 1, 1, 0, 0))
      expect(helper.survey_formatted_response_created_at(response)).to eq('January 01, 2021 at 01:00 AM')
    end
  end
